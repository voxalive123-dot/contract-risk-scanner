...
export default function Page() {
  ...
  useEffect(() => {
    let cancelled = false;
    fetch("/api/internal/ops/organizations", { cache: "no-store" })
      .then(async (response) => {
  ...
  const preview = data ? (data as Record<string, unknown>)["organizations"] : null;

  return (
    <InternalShell eyebrow="Organisations" title="Organisation management.">
