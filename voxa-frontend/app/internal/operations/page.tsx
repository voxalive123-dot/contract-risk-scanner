...
type OrgSummary = {
  ...
  subscription: { plan_name: string; status: string; source: string } | null;
  billing_customer_reference: { provider: string; external_customer_id: string; billing_email: string | null } | null;
  member_count: number;
...
export default function InternalOperationsPage() {
  ...
  async function recordOperatorNote() {
    ...
    const response = await fetch("/api/internal/ops/workflow", {
      ...
      body: JSON.stringify({ type: "operator_note", org_id: selectedOrgId, reason: operatorReason }),
    });
        ...
        <section className="mx-auto max-w-[1360px] px-6 py-10 md:px-8">
          <div className="rounded-[2rem] border border-[#dfd0b6] bg-[#fffaf0] p-8 shadow-[0_22px_60px_rgba(75,55,25,0.10)] md:p-10">
            ...
            <p className="mt-6 max-w-3xl text-base leading-8 text-neutral-700">
              Operator visibility across organisations, memberships, entitlement truth, billing references, usage, scans, invites, and recent operational signals. Workflow actions are narrow, audited, and do not become entitlement authority.
            </p>
            ...
            <div className="mt-8 grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
              <section className="rounded-[1.25rem] border border-[#d8c49e] bg-[#fbf3e5] p-6">
                <div className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[#8a6a34]">Organisations</div>
                  ...
                  <div className="mt-5 space-y-5 text-sm leading-6 text-neutral-700">
                    ...
                    <div className="rounded-xl border border-[#d2bd96] bg-[#fffdf8] p-4">
                      ...
                      <textarea
                        ...
                        placeholder="Record investigation note or operator reason"
                      ...
                      <button
                        type="button"
                        disabled={actionLoading}
                        onClick={recordOperatorNote}
                        className="mt-3 rounded-xl bg-[#11110f] px-4 py-2 text-sm font-semibold text-stone-100 transition hover:bg-[#1b1a17] disabled:opacity-50"
                      >
